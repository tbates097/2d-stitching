#!/usr/bin/env python3
"""
COMPARE_MULTIZONE_STEP3 - Compare multi-zone Step 3 outputs between MATLAB and Python
"""
...
def compare_multizone_step3():
    """Compare multi-zone Step 3 output files"""
    
    print("=" * 60)
    print("MULTI-ZONE STEP 3: MATLAB vs PYTHON COMPARISON")
    print("=" * 60)
    
    octave_file = "multizone_step3_output_octave.txt"
    python_file = "multizone_step3_output_python.txt"
    
    # Check if both files exist
    if not os.path.exists(octave_file):
        print(f"❌ MATLAB output file missing: {octave_file}")
        print("Please run: test_multizone_step3_octave")
        return False
    
    if not os.path.exists(python_file):
        print(f"❌ Python output file missing: {python_file}")
        print("Please run: python multizone_step3_stitch_zones.py")
        return False
    ...
    stitching_params = ['totalZones', 'nonZeroPoints', 'maxAvgCount']
    grid_params = ['fullGridSize_rows', 'fullGridSize_cols']
    coordinate_params = ['X_min', 'X_max', 'Y_min', 'Y_max']
    error_params = ['Ax1Err_min', 'Ax1Err_max', 'Ax2Err_min', 'Ax2Err_max']
    env_params = ['airTemp_mean', 'matTemp_mean']
                ...
                if octave_val == "MISSING":
                    ...
                    try:
                        ...
                        if oct_num == py_num:
                            ...
                            # Use more relaxed tolerance for stitching results
                            tolerance = 1e-8 if 'Err' in key else 1e-10
                            
                            if diff < tolerance or rel_diff < tolerance:
                                print(f"  ✓ {key:20}: {octave_val} (diff: {diff:.2e})")
                                group_matches += 1
                            else:
                                print(f"  ✗ {key:20}: MATLAB={octave_val}, Python={python_val} (diff: {diff:.2e}, rel: {rel_diff:.2e})")
    ...
    # Compare each group
    stitch_matches, stitch_diffs = compare_group("STITCHING RESULTS", stitching_params)
    grid_matches, grid_diffs = compare_group("GRID PARAMETERS", grid_params)
    coord_matches, coord_diffs = compare_group("COORDINATE RANGES", coordinate_params)
    error_matches, error_diffs = compare_group("ERROR STATISTICS", error_params)
    env_matches, env_diffs = compare_group("ENVIRONMENT DATA", env_params)
    
    # Collect all results
    matches = stitch_matches + grid_matches + coord_matches + error_matches + env_matches
    differences = stitch_diffs + grid_diffs + coord_diffs + error_diffs + env_diffs
    
    # Summary
    total = len(stitching_params + grid_params + coordinate_params + error_params + env_params)
    available_total = len([k for k in stitching_params + grid_params + coordinate_params + error_params + env_params if k in all_keys])
    ...
    if success:
        print("\n🎉 PERFECT MATCH! Multi-zone Step 3 implementations are identical.")
    else:
        print(f"\n⚠️  {len(differences)} differences found. Need investigation.")
        
        # Provide guidance on acceptable differences
        error_diffs = [d for d in differences if 'Err_' in d]
        if error_diffs:
            print("\n💡 NOTE: Small differences in error statistics may be acceptable")
            print("   due to numerical precision in the stitching algorithm.")
    
    # Check for any extra parameters not in our groups
    known_params = set(stitching_params + grid_params + coordinate_params + error_params + env_params)
    ...
    with open('multizone_step3_comparison_report.txt', 'w') as f:
        f.write("MULTI-ZONE STEP 3 COMPARISON REPORT\n")
        f.write("=" * 40 + "\n\n")
    ...
    print(f"\nDetailed report saved to: multizone_step3_comparison_report.txt")
    print("=" * 60)
...
if __name__ == "__main__":
    success = compare_multizone_step3()
