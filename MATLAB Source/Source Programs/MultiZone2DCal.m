...
CalFile = '630518-1-1-2D.cal';
UserUnit = 'ENGLISH';  % user units = 'METRIC' or 'ENGLISH'
                      % correction units are "user units"/1000

writeOutputFile = 1;
OutFile = '630518-1-1-2DAccuracy.dat';                      

%%%%%%%%%%%%%%%% Set Calibration Zone Parameters %%%%%%%%%%%%%%%%%%%%
numRow = 1;                 % number of calibration zone rows (how many zones in Axis 2 (Step axis) direction)
numCol = 2;                 % number of calibration zone columns (how many zones in Axis 1 (Scan axis) direction)
travelAx1 = ([-200/25.4 200/25.4]);      % full calibrated travel for Axis 1 (scan axis)
travelAx2 = ([-75/25.4 75/25.4]);      % full calibrated travel for Axis 2 (step axis)
...
for i = 1:numRow
    for j = 1:numCol
        counter = counter + 1;
        fName(i,j) = {sprintf('630518-1-1-XY2DCZ%u.dat', counter)};
    end
end
...
% accuracy and orthogonality with A3200ACC2DMultiZone.m.  A structure
...
for i = 1:numRow
    for j = 1:numCol
        if (i == 1) && (j == 1)     % open data for the first zone
            data = A3200Acc2DMultiZone(char(fName(i,j)));
        ...
        data = A3200Acc2DMultiZone(char(fName(i,j+1)));
        zoneCount = zoneCount + 1;
...
%               legend('Master Zone', 'Slave Zone','location','southoutside')
    ...
    data = A3200ACC2DMultiZone(char(fName(i+1,1)));
    zoneCount = zoneCount + 1;
...
[textBox1, textBox2, textBox3] = AerotechFooter(1);
%create cell array of strings for text box 1
txt1(1) = {'\bfResults\rm'};
txt1(2) = {' '};
txt1(3) = {sprintf('%s Direction Error: \\pm%1.2f%s', Ax1Name, Ax1amplitude, errUnit)};
txt1(4) = {sprintf('%s Direction Error: \\pm%1.2f%s', Ax2Name, Ax2amplitude, errUnit)};
txt1(5) = {sprintf('%s-%s Orthogonality: %1.2farc-sec', Ax1Name, Ax2Name, orthog)};
txt1(11) = {'\itAerotech Confidential\rm'};
%add results information to text box 1
set(textBox1, 'String', txt1)

%create cell array of strings for text box 2
txt2(1) = {'\bfComments\rm'};
txt2(2) = {' '};
txt2(3) = {sprintf('Machine: %s', model)};
txt2(4) = {sprintf('Serial No.: %s', SN)};
txt2(5) = {sprintf('Date: %s', char(fileDate(1,1)))};
txt2(6) = {sprintf('Operator: %s', operator)};
tempTxt = char(fName(1,1));
txt2(7) = {sprintf('Files: %s ~ %u.dat', tempTxt(1:(end-4)), counter)};
txt2(8) = {' '};
if ~isempty(comment)
    txt2(9) = comment(1);
end
%add results information to text box 2
set(textBox2, 'String', txt2)

%create cell array of strings for text box 3
txt3(1) = {'\bfEnvironmental\rm'};
txt3(2) = {' '};
if ~isempty(matTemp)
    txt3(3) = {sprintf('Min Air Temperature: %1.2f\\circC', min(min(airTemp)))};
    txt3(4) = {sprintf('Max Air Temperature: %1.2f\\circC', max(max(airTemp)))};
    txt3(6) = {sprintf('Min Material Temperature: %1.2f\\circC', min(min(matTemp)))};
    txt3(7) = {sprintf('Max Material Temperature: %1.2f\\circC', max(max(matTemp)))};
else
    txt3(3) = {sprintf('Air Temperature: -')};
    txt3(4) = {sprintf('Material Temperature: -')};
end
txt3(5) = {' '};
if ~isempty(expandCoef)
    txt3(9) = {sprintf('Expansion Coef: %s ppm /\\circC', expandCoef)};
    txt3(10) = {'Reference Temperature: 20.0\circC'};
end
%add results information to text box 3
set(textBox3, 'String', txt3)
